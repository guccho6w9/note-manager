...
export class NotesController {
  ...
  @Post()
  async create(
    @Body() noteData: { title: string; content: string; tagIds: number[] },
  ): Promise<Note> {
  ...
  @Get()
async findAll(@Query('active') active: string, @Query('tagId') tagId?: string) {
  const isActive = active === 'true';
  return this.notesService.findAll(isActive, tagId ? +tagId : undefined);
}
